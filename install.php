...
\$dbprefix = '$mysqlData[dbprefix]';";
	...
	elseif($_POST["op"]["pass1"]!=$_POST["op"]["pass2"]) $div.="Ошибка! Пароли не совпадают<br>";
		...
		elseif(register_user($_POST["op"]["login"],$_POST["op"]["email"],$_POST["op"]["pass1"],"",10)) $div.="Администратор ".$_POST["op"]["login"]." создан<br>";
...
<div style='border:1px solid gray;border-radius:10px;padding:15px;'>	
				URL (без / в конце)
				<br>
				<input name='op[siteurl]' style='width:250px;' value='<?="http://$_SERVER[HTTP_HOST]"?>' /><br>
				Название
				<br>
